...
class WTRBlockReacherEnv(MujocoEnv, utils.EzPickle):
    '''
    Changes Made: 
    1. theta_normalised is removed from the observation space (redundant observation)
    2. Torque control is changed to velocity control 
    3. Action space is limited to [-1,1]
    4. Denormalization code is added 
    5. finalVelocity reward is changed to penalty if the robot does not reach 0 velocity upon reaching goal. 
    '''
    metadata = {
        "render_modes": [
            "human",
    ...
    def denormalize(self,v,w):
        normalised_v = (0.5 * (v+1) * (2* self.max_v)) - self.max_v
        normalised_w = (0.5 * (w+1) * (2* self.max_w)) - self.max_w
    ...
    def _get_observation_space(self):
        ...
        min_obs = np.hstack((min_gd,min_rel,min_yaw,min_vel))
        max_obs = np.hstack((max_gd,max_rel, max_yaw, max_vel))
    ...
    def checkFlags(self):
        ...
        x,y = robot_pos
        conditions = [ (x < -self.config["mujoco"].get("env_length") or x > self.config["mujoco"].get("env_length")),
                       (y < -self.config["mujoco"].get("env_length") or y > self.config["mujoco"].get("env_length"))]
        if any(conditions):
            self.failed = True
    ...
    def reset(self,seed=None,options=None):
        ...
        ## Ball coordinates
        cx,cy = np.random.randint(low=-self.config["mujoco"].get("coordinate_limits"),high=self.config["mujoco"].get("coordinate_limits")) + \
    ...
    def step(self,action):
        ## Action is an array of size 2 containing the v and w of the wheels
        v,w = self.denormalize(*action)
        ctrlvels = self.differential_drive(np.array([v,w]))
        ...
        ## Check for ctrlvels
        finalVelocity = abs(self.data.qvel[6:9] - self.vf) if self.reached else 0
